...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
  ...
  BTreeNode *cur = children_[idx];

  // change parent node, replace the "root" with left.back()
  int temp = this->elements_[idx - 1];
  this->elements_[idx - 1] = prev->elements_.back();
  prev->elements_.pop_back();

  // change right child
  cur->elements_.insert(cur->elements_.begin(), temp);
